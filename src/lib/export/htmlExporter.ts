...
interface LessonExportData {
  ...
  assets?: VisualAssets;
  /**
   * Rubric result stamped onto the page. A downloaded handout is uploaded to an
   * LMS by hand and is just as district-facing as one we push ourselves, so it
...
export function generateStudentHTML(data: LessonExportData): string {
  const { title, content, englishContent, group, generatedDate, assets, conformance } = data;
...
<body>
  <div class="lesson-container">
    ...
    ${conformance ? buildConformanceFooterHTML(conformance) : ''}
...
export async function downloadAllAsZip(
  ...
  groupContents.forEach(({ group, content, englishContent }) => {
    const html = generateStudentHTML({ title, content, englishContent, group, generatedDate, assets, conformance });
